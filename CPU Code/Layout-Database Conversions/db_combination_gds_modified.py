...
def examine_db_content(db_file):
    """Print detailed information about a database's content"""
    ...
    # Check processed_polygons table
    try:
        cursor.execute("SELECT COUNT(*) FROM processed_polygons")
        count = cursor.fetchone()[0]
        print(f"Total rows in processed_polygons: {count}")
        
        # Get sample data
        cursor.execute("SELECT polygon_id, x, y FROM processed_polygons LIMIT 10")
        sample = cursor.fetchall()
        print(f"Sample data: {sample}")
        
        # Get coordinate ranges
        cursor.execute("SELECT MIN(x), MAX(x), MIN(y), MAX(y) FROM processed_polygons")
        ranges = cursor.fetchone()
        print(f"Coordinate ranges: x=[{ranges[0]}, {ranges[1]}], y=[{ranges[2]}, {ranges[3]}]")
    except Exception as e:
        print(f"Error examining processed_polygons: {e}")
...
def combine_divided_dbs_to_gds(db_directory, output_gds, prefix=""):
    ...
    # Add this diagnostic code to examine the first two DB files
    for db_file in db_files[:2]:  # Check first two files
        examine_db_content(db_file)
...
if __name__ == "__main__":
    ...
    output_gds = "GDS Files/multiple_polygon_modified.gds"
