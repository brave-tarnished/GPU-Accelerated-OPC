...
void processDBFile(const std::string& dbFile, 
    float edgeStepSize, 
    float cornerStepSize,
    int horizontalCornerFragments, 
    int verticalCornerFragments,
    float shiftDist,
    std::atomic<int>& completedFiles,
    int totalFiles) {
    ...
    try {
        ...
        std::cout << "Processed " << dbFile << " (" << completed << "/" << totalFiles << ")" << std::endl;
    } 
    catch (const std::exception& e) {
        std::cerr << "Error processing " << dbFile << ": " << e.what() << std::endl;
    }
}
...
int main() {
    ...
    // Get all DB files from a directory
    std::string dbDirectory = "DB Files/Divided/";
    ...
    if (dbFiles.empty()) {
        std::cerr << "No DB files found in directory: " << dbDirectory << std::endl;
        return 1;
    }
    ...
    // Limit number of concurrent threads to avoid overloading the system
    unsigned int maxConcurrentThreads = std::min(numThreads, static_cast<unsigned int>(dbFiles.size()));
    
    std::cout << "Processing " << dbFiles.size() << " DB files using " 
              << maxConcurrentThreads << " threads" << std::endl;
    
    // Process files in batches to control memory usage
    for (size_t i = 0; i < dbFiles.size(); i += maxConcurrentThreads) {
        size_t end = std::min(i + maxConcurrentThreads, dbFiles.size());
        threads.clear();
        
        // Launch a batch of threads
        for (size_t j = i; j < end; ++j) {
            threads.emplace_back(processDBFile, 
                               dbFiles[j], 
                               edgeStepSize, 
                               cornerStepSize,
                               horizontalCornerFragments, 
                               verticalCornerFragments,
                               shiftDist,
                               std::ref(completedFiles),
                               static_cast<int>(dbFiles.size()));
        }
