...
class Timer {
    ...
    public:
        ...
        ~Timer() {
            ...
            std::cout << operation_name << " took " << duration.count() << " milliseconds." << std::endl;
        }
    };    
    ...
    int main() {
        try {
            ...
            const float shiftDist = 0.05f; //0.01f for 0.1um
    
            PolygonData input;
            {
                Timer read_timer("Database read");
                input = readDB(dbFile);
            }
            ...
            PolygonData output;
            {
                Timer process_timer("Polygon processing");
                output = processAndShift(input, edgeStepSize, cornerStepSize, 
                                         horizontalCornerFragments, verticalCornerFragments,
                                         shiftDist);
            }
    
            {
                Timer write_timer("Database write");
